...
/*   Updated: 2025/07/12 10:22:22 by jpmesquita       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lib.h"

int	gnlstrlen(char *s)
{
	int	i;
...
char	*ft_strjoin(char *line, char *read_buffer)
{
	...
	str = malloc((gnlstrlen(line) + gnlstrlen(read_buffer) + 1) * sizeof(char));
	if (!str)
	{
		free(str);
		return (NULL);
